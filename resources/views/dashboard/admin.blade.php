...
<div class="w-full mt-8 px-4">
    ...
    <div class="mt-8 flex flex-wrap gap-4">
        <!-- Gráfico -->
        <div style="width: 49.5rem;height:35rem" class=" lg:w-2/3 bg-white p-6 rounded shadow-md" style="height: 65vh">
            <canvas id="dashboardChart"></canvas>
        </div>
        <!-- Card de Pagamentos -->
        {{-- <div style="background-color:#1a187900" class="w-full lg:w-1/3 text-center border border-green-300 px-8 py-6 rounded shadow-md flex flex-col justify-center">
            <h3 class="text-green-700 uppercase font-bold flex items-center justify-center gap-4">
                
                <span class="text-3xl font-bold text-white">{{ number_format($payments->sum('amount'), 2) }} MZN</span>
            </h3>
            <p class="text-green-600 mt-2 text-lg">Total de Pagamentos</p>
            <p class="text-green-500 text-sm">Número de Transações: <strong>{{ count($payments) }}</strong></p>
        </div> --}}
        <div style="height: 10rem;margin-left:0.5rem;background-color:#1a1879" class="w-full sm:w-1/3 bg-gradient-to-r  from-blue-500 to-blue-700 text-white text-center border border-blue-600 px-8 py-6 rounded-lg shadow-lg">
            <h3 class="uppercase font-bold flex items-center justify-center gap-4">
                <svg style="color: white" class="h-10 w-10 fill-current" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
                ...
                <div class="text-left" style="margin-left:0.5rem">
                    ...
                    <p class="text-3xl font-bold mt-1" style="color: white">MZN {{ number_format($payments->sum('amount'), 2, ',', '.') }}</p>
                </div>{{--#EFB036--}}
            </h3>
        </div>
    </div>
