                ...
                @else
                    <form action="{{ route('notes.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <button style="margin-top:1rem" type="submit"
                            class="bg-blue-600 text-white px-6 py-3 rounded-md hover:bg-blue-500 focus:ring-2 focus:ring-blue-400 transition-colors">
                            Registrar Notas
                            </buttonn>
                    </form>
                @endif
                ...
                @else
                    <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
                        ...
                        @foreach ($student->notes as $note)
                                ...
                                @if ($note->$type !== null)
                                    ...
                                    <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-gray-300">
                                        ...
                                        <div class="w-2/12 flex items-center justify-end px-3">
                                            <a onclick="openEditModal({{ $note->id }})" href="#"
                                                class="ml-1" type="button" data-bs-toggle="modal"
                                                data-bs-target="#editModal{{ $note->id }}">
                                                <svg class="h-6 w-6 fill-current text-green-600" aria-hidden="true"
                                                    focusable="false" data-prefix="fas" data-icon="pen-square"
                                                    class="svg-inline--fa fa-pen-square fa-w-14" role="img"
                                                    xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                                                    <path fill="currentColor"
                                            ...
                                            <a href="{{ route('notes.delete', ['id' => $note->id]) }}"
                                                class="deletestudent ml-1 bg-red-600 block p-1 border border-red-600 rounded-sm"
                                                title="Delete">
                                                <svg class="h-3 w-3 fill-current text-gray-100" aria-hidden="true"
                            ...
                            <div id="editModal{{ $note->id }}" data-bs-keyboard="false" data-bs-backdrop="static"
                                tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true"
                                class="modal-bg hidden fixed top-0 left-0 right-0 bottom-0 w-full h-full overflow-auto z-50 flex items-center justify-center">
                                <div
                                    class="bg-white relative p-10 max-w-lg w-full mx-4 sm:mx-auto my-10 sm:my-32 shadow-lg rounded-lg">
                                    <div class="absolute top-0 right-0 m-3 text-red-600 cursor-pointer">
                                        <button onclick="closeModal('editModal{{ $note->id }}')">
                                            <svg class="w-6 h-6 stroke-current" aria-hidden="true" focusable="false"
                                    ...
                                    <h2 class="text-xl font-semibold text-gray-800 mb-6 text-center">Formulario de Edicao
                                        de Dados</h2>

                                    {{-- ?Start the update form --}}
                                    <div class="w-full md:w-2/2 bg-white rounded p-6">
                                        <form
                                            action="{{ route('notes.update', [
                                                'id' => $note->id,
                                            ]) }}"
                                            method="POST">
                                            @csrf
                                            <div class="mb-4">
                                                ...
                                                <select name="subject_id"
                                                    class="w-full bg-gray-200 border rounded py-2 px-3">
                                                    <option value="{{ $note->subject_id }}">{{ $note->subject->name }}
                                                    </option>
                                                    @foreach ($student->class->subjects as $subject)
                                                        <option value="{{ $subject->id }}">{{ $subject->name }}</option>
                                                    @endforeach
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <input value="{{ $note->first }}" name="first" type="number"
                                                    class="w-full bg-gray-200 border rounded py-2 px-3">
                                                @error('note')
                                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                                @enderror
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <input value="{{ $note->second }}" name="second" type="number"
                                                    class="w-full bg-gray-200 border rounded py-2 px-3">
                                                @error('note')
                                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                                @enderror
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <input value="{{ $note->third }}" name="third" type="number"
                                                    class="w-full bg-gray-200 border rounded py-2 px-3">
                                                @error('note')
                                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                                @enderror
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <input value="{{ $note->work }}" name="work" type="number"
                                                    class="w-full bg-gray-200 border rounded py-2 px-3">
                                                @error('note')
                                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                                @enderror
                                            ...
                                            <div class="flex justify-end mt-6">
                                                ...
                                                <button style="margin-left:0.5rem" type="submit"
